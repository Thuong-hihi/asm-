...
const m = movies.map(item => item.cast).flat();
const casts = m.map((item, index) => {
    return {
        id: Date.now() + index,
        name: item
    }
...
const allGenres = movies.map(gen => gen.genres).flat();
const unique = new Set(allGenres);

const genArray = [...unique];
const genres = genArray.map((gen, index) => {
    return {
        id: Date.now() + index,
        name: gen
    }
